...
- **別館 ｜ ANNEX（番号なき者たち）** — 本館の図録番号体系の外側にある、番号を持たないグループ群の展示室。命名は帰属追跡の系譜であるという視点から、Mandiant の4系列（APT／FIN／UNC／TEMP）と昇格の階梯、CrowdStrike の国別動物サフィックス、Microsoft の気象ファミリー名、同一グループが各社で異なる名で呼ばれる実例を、美術館の解説パネルとして並べます。あわせて、コードネームと MITRE ID を掲げる無番の標本カタログを収録し、各プレートをひらくとベンダーごとの呼称・代表的な事件（年表）・関連するマルウェア・出典、そして「本館の関連図録」へのリンク（本館モーダルへ遷移）が開きます。
